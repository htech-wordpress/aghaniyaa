...
export interface Address {
    ...
    value: string; // e.g. "Sohrab Hall, Pune..."
    mapLink?: string;
    phone?: string;
}
...
export async function getCompanyStats(): Promise<CompanyStats> {
    ...
    const snap = await get(child(ref(db), 'settings/companyStats'));
    if (!snap.exists()) return defaultStats;
    return snap.val() as CompanyStats;
}

export async function updateCompanyStats(stats: CompanyStats): Promise<void> {
    const db = ensureDatabase();
    await set(ref(db, 'settings/companyStats'), stats);
}
